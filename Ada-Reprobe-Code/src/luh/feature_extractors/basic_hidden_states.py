...
class FeatureExtractorBasicHiddenStates(FeatureExtractorBase):
    def __init__(self, orig_base_model, layer_nums=[-1], token_alignment="previous", **kwargs):
        ...
        if token_alignment not in {"previous", "current"}:
            raise ValueError(
                "token_alignment must be 'previous' (legacy next-token shift) "
                "or 'current' (the hidden state of each claim token)"
            )
        self._token_alignment = token_alignment
    ...
    def __call__(self, llm_inputs, llm_outputs):
        ...
        if is_training:
            ...
            token_slice = slice(None, -1) if self._token_alignment == "previous" else slice(1, None)
            res = torch.cat(
                [llm_outputs["hidden_states"][layer][:, token_slice, :] for layer in self._layer_nums], dim=-1
            )
